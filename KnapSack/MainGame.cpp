...
MainGame::~MainGame()
{
	...
	// Don't forget to delete the good androids and Evil Androids
...
void MainGame::initSystems()
{
	...
	// Create our window
	_window.create("KnapSack Save Android", _screenWidth, _screenHeight, 0);
...
void MainGame::updateAgents()
{
	...
	// Update Evil Androids collisions
	for (int i = 0; i < _zombies.size(); i++)
	{
		// Collide with other Evil Androids
		...
		{
			Bengine::fatalError("YOU LOSE!! Even you are now AFFECTED by EVIL ANDROID !!");
		}
	}
	...
	// Dont forget to update Evil Androids
}
...
void MainGame::updateBullets()
{
	...
	// Collide with humans and Evil Androids
	for (int i = 0; i < _bullets.size(); i++)
	{
		wasBulletRemoved = false;
		// Loop through Evil Androids
		for (int j = 0; j < _zombies.size();)
		{
			// Check collision
			if (_bullets[i].collideWithAgent(_zombies[j]))
			{
				// Damage Evil Androids, and kill it if its out of health
				if (_zombies[j]->applyDamage(_bullets[i].getDamage()))
				{
					// If the Evil Androids died, remove him
					delete _zombies[j];
				...
				// Since the bullet died, no need to loop through any more Evil Androids
				break;
			}
			else
			{
...
void MainGame::checkVictory()
{
	...
	// If all Evil Androids are dead we win!
	if (_zombies.empty()) {
		// Print victory message
		std::printf("*** You win! ***\n You killed %d Good Androids and %d Evil Androids. There are %d/%d Good Androids remaining",
